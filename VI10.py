...
def run_on_hardware(circuit, shots=SHOTS, ibm_token=None, backend_name="ibm_brisbane"):
...
def main(light_mode=True, use_hardware=True, ibm_token=None, debug_mode=False, backend_name="ibm_brisbane"):
    ...
    print(f"Estimated runtime: IBM Quantum queue time + local processing (~{NUM_RUNS * 3 / 60:.1f} minutes on a 4-core CPU).")
    ...
    # Optional: List available IBM Quantum backends
    if use_hardware and ibm_token:
        try:
            service = QiskitRuntimeService(channel="ibm_quantum", token=ibm_token)
            print("Available backends:")
            for backend in service.backends():
                print(f"Backend: {backend.name}, Qubits: {backend.configuration().n_qubits}, Status: {backend.status().status}")
        except Exception as e:
            print(f"Error listing backends: {str(e)}. Continuing with specified backend: {backend_name}")
    ...
    start_time = time.time()
    if use_hardware and ibm_token:
        print(f"Running simulation on IBM Quantum backend: {backend_name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--backend_name', type=str, default="ibm_brisbane", help="IBM Quantum backend name (e.g., ibm_brisbane)")
