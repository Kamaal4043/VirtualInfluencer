...
# Constants (optimized for efficiency)
NUM_VI_QUBITS = 4
NUM_CONSUMER_QUBITS = 4
NUM_CONTEXT_QUBITS = 4
TOTAL_QUBITS = NUM_VI_QUBITS + NUM_CONSUMER_QUBITS + NUM_CONTEXT_QUBITS
SHOTS = 10000  # Default, adjustable in light mode
NUM_RUNS = 60   # Default, adjustable in light mode
...
def apply_zne(circuit, measurement_func, noise_levels=[1, 2, 3]):
    ...
    return coeffs[1]  # Zero-noise result
...
# Enhanced classical benchmarking
def benchmark_classical(results_df):
    """Compare quantum results against classical neural network, including runtime and purchase intention."""
    ...
    # Estimate quantum runtime from results_df metadata (if available) or simulate one condition
    quantum_time = results_df.get('runtime', classical_time * 2)  # Placeholder if not tracked
    ...
    metrics['runtime'] = {
        'quantum': quantum_time * NUM_RUNS,  # Scale by number of runs
...
# New: Run on quantum hardware
def run_on_hardware(circuit, shots=SHOTS, ibm_token=None, backend_name="ibmq_qasm_simulator"):
    """Execute circuit on IBM Quantum hardware using Qiskit Runtime."""
    if QiskitRuntimeService is None or Sampler is None:
        print("Qiskit IBM Runtime not installed. Falling back to simulator.")
        return None
    ...
    try:
        ...
        print(f"Hardware execution failed: {str(e)}. Using simulator.")
        return None
...
def visualize_results(results_df, stats_summary, classical_metrics=None):
    """Generate comprehensive visualizations, including R² comparison."""
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    influencer_types = results_df['influencer_type'].unique()
    ...
    # New: R² comparison visualization
...
def batch_process(tasks, batch_size=None, depth=DEPTH, noise_prob=0.02):
    """Process tasks in batches with dynamic sizing."""
    if batch_size is None:
        batch_size = max(5, mp.cpu_count() * 2)  # Dynamic based on CPU cores
    results = []
...
# Main execution
def main(light_mode=False, use_hardware=False, ibm_token=None):
    """Run the QSPF simulation and analysis."""
    global SHOTS, NUM_RUNS
    if light_mode:
        SHOTS = 5000
        NUM_RUNS = 30
        print("Running in light mode: SHOTS=5000, NUM_RUNS=30, Bayesian samples=200")
    
    print("WARNING: This simulation is computationally intensive (12 qubits).")
    print(f"Estimated runtime: ~{NUM_RUNS * 5 / 60:.1f} minutes on a 4-core CPU.")
    ...
    results_df['runtime'] = quantum_time / NUM_RUNS  # Per-run runtime
...
if __name__ == "__main__":
    main(light_mode=False, use_hardware=False, ibm_token=None)